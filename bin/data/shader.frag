...
void main (void)
{
    vec2 uv = texCoordVarying/(40000.0*cloudScale);
...
//    float color2 = fbm(uv-10.5+iGlobalTime*0.00002*timeScale);
	...
	float color1 = fbm(uv-10.5+iGlobalTime*0.00002*timeScale);
	float color1 = fbm(uv-10.5+iGlobalTime*0.00002*timeScale);
    ...
    float cloudsFormComb = clamp(clouds1+clouds2, 0.0,1.0);
    
    vec4 skyCol = vec4(0.6,0.8,1.0,1.0);
    float cloudCol = clamp(clamp(1.0-pow(color1,1.0)*0.2, 0.0, 1.0 )*bright, 0.0, 1.0);
    vec4 clouds1Color = vec4(cloudCol,cloudCol,cloudCol,1.0);
    vec4 clouds2Color = mix(clouds1Color,skyCol,0.25);
    vec4 cloudColComb = mix(clouds1Color,clouds2Color,clamp((clouds2-clouds1), 0.0,1.0));
    
	outputColor = mix(skyCol,cloudColComb,cloudsFormComb);
	outputColor = texture(iChannel0, texCoordVarying);
}